...
class L2NormPenaltyNode(object):
    ...
    def forward(self):
        ...
        # create d_out with shape of self.out
        self.d_out = np.zeros(self.out.shape)

        return self.out

    def backward(self):
        temp = 2 * self.l2_reg * self.w.out * self.d_out
        self.w.d_out += temp
...
class AffineNode(object):
    ...
    def forward(self):
        ...
        self.d_out = np.zeros(self.out.shape)  # this node can actually be in init?
...
class TanhNode(object):
    ...
    def backward(self):
        d = self.d_out * (1 - np.power(self.out, 2))
        self.a.d_out += d
...
class SoftmaxNode(object):
    ...
    def forward(self):
        print(self.z.out)
        e_z = np.exp(self.z.out - np.max(self.z.out))
        self.out = e_z / e_z.sum()
        print(self.out)
        self.d_out = np.zeros((self.out.shape[0], self.z.out.shape[0]))
        return self.out

    def backward(self):
        d = np.zeros((self.out.shape[0], self.z.out.shape[0]))

        for i in range(len(self.out)):
            for j in range(len(self.z.out)):
                if i == j:
                    d[i, j] = self.out[i] * (1 - self.z.out[i])
                else:
                    d[i, j] = -self.out[i] * self.z.out[j]
