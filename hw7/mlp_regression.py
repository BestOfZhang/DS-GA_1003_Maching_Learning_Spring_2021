...
class MLPRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 5000):
        ...
        self.affine_node = nodes.AffineNode(W=self.W1,
                                            x=self.x,
                                            b=self.b1,
                                            node_name='affine')
        self.tanh_node = nodes.TanhNode(a=self.affine_node,
                                        node_name='tanh')
        self.pred = nodes.VectorScalarAffineNode(x=self.tanh_node,
                                                       w=self.w2,
                                                       b=self.b2,
                                                       node_name='prediction')
        self.J = nodes.SquaredL2DistanceNode(a=self.pred,
                                             b=self.y,
                                             node_name='objective function')
